...
//                 script {
//                     withCredentials([file(credentialsId: 'FileKey', variable: 'secretFile')]) {
//                         // Disable host key checking by setting the SSH option `StrictHostKeyChecking` to `no`
//                         sh '''
//                         ssh -o StrictHostKeyChecking=no -tt -i $secretFile ${AWS_EC2_USER}@${AWS_EC2_HOST} yes <<EOF
//                             docker pull ${DOCKER_IMAGE}
//                             docker stop nodejs-app || true
//                             docker rm nodejs-app || true
//                             docker run -d --name nodejs-app -p 80:80 ${DOCKER_IMAGE}
